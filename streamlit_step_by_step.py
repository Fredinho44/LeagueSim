...
-------------------------
A step-by-step interface for the ModelCA League Simulator with expandable sections
...
st.set_page_config(page_title="ModelCA League Simulator", layout="wide")
...
# Main title
st.title("⚾ ModelCA League Simulator")
st.markdown("---")
...
try:
    _idx = int(st.session_state.step)
except Exception:
    _idx = 1
_idx = max(1, min(_idx, 4))
st.sidebar.markdown("**Current Step:** " + _steps_display[_idx])
